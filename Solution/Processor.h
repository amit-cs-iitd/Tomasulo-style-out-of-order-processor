...
public:
    ...
    void loadProgram(const std::string& filename) {
        ...
        auto parse_int = [](const std::string& t) { return std::stoi(t); };
        ...
        while (std::getline(file, line)) {
            ...
            while (true) {
                ...
                if (!lbl.empty()) code_labels[lbl] = static_cast<int>(raw_instructions.size());
                line = trim(line.substr(cpos + 1));
                if (line.empty()) break;
            }
            if (!line.empty()) raw_instructions.push_back(line);
        }

        auto parse_reg = [](const std::string& r) {
            if (r.size() < 2 || r[0] != 'x') throw std::runtime_error("Bad register");
        ...
        auto parse_op = [](const std::string& op) {
            ...
            if (!M.count(low)) throw std::runtime_error("Bad opcode");
            return M[low];
        };
        ...
        for (int i = 0; i < static_cast<int>(raw_instructions.size()); i++) {
            ...
            switch (ins.op) {
                ...
                case OpCode::REM: case OpCode::SLT: case OpCode::AND: case OpCode::OR: case OpCode::XOR:
                    ins.dest = parse_reg(toks[1]);
                    ins.src1 = parse_reg(toks[2]);
                    ins.src2 = parse_reg(toks[3]);
                    break;
                case OpCode::ADDI: case OpCode::SLTI: case OpCode::ANDI: case OpCode::ORI: case OpCode::XORI:
                    ins.dest = parse_reg(toks[1]);
                    ins.src1 = parse_reg(toks[2]);
                ...
                case OpCode::LW: {
                    ...
                    if (mem_labels.count(ofs)) ins.imm = mem_labels[ofs];
                    else ins.imm = parse_int(ofs);
                    base = parse_reg(reg);
                    ins.src1 = base;
                    break;
                }
    ...
    UnitType unitForOp(OpCode op) const {
        if (op == OpCode::MUL) return UnitType::MULTIPLIER;
        if (op == OpCode::DIV || op == OpCode::REM) return UnitType::DIVIDER;
        if (op == OpCode::LW || op == OpCode::SW) return UnitType::LOADSTORE;
        if (isBranchOp(op)) return UnitType::BRANCH;
        if (op == OpCode::AND || op == OpCode::OR || op == OpCode::XOR ||
            op == OpCode::ANDI || op == OpCode::ORI || op == OpCode::XORI) return UnitType::LOGIC;
        return UnitType::ADDER;
    }

    ExecutionUnit* getUnit(UnitType u) {
        for (auto& x : units) if (x.name == u) return &x;
        return nullptr;
    }

    ROBEntry* getROBByTag(int tag) {
        auto it = rob_tag_to_index.find(tag);
        if (it == rob_tag_to_index.end()) return nullptr;
        int idx = it->second;
        if (!ROB[idx].valid || ROB[idx].tag != tag) return nullptr;
        return &ROB[idx];
    }

    int allocROB(const Instruction& ins, int predicted_next_pc) {
        if (rob_count >= static_cast<int>(ROB.size())) return -1;
        int idx = rob_tail;
        int tag = next_tag++;
        ROB[idx] = ROBEntry{};
        ROB[idx].valid = true;
    ...
    void stageFetch() {
        if (fetched.valid) return;
        if (pc < 0 || pc >= static_cast<int>(inst_memory.size())) return;
    ...
    void readOperand(int reg, int& value, int& tag, bool& ready) {
        if (reg == 0) {
            value = 0; tag = -1; ready = true; return;
        }
        int dep = RAT[reg];
        if (dep == -1) {
            value = ARF[reg];
    ...
    void stageDecode() {
        ...
        if (rob_count >= static_cast<int>(ROB.size())) return;
        if (needs_rs) {
            if (unit_t == UnitType::LOADSTORE) {
                if (!lsq->hasSpace()) return;
            } else {
                ExecutionUnit* u = getUnit(unit_t);
                if (!u || !u->hasSpace()) return;
            }
        }
        ...
        auto fill_reg_src = [&](int reg, bool is_first) {
            int v = 0, t = -1; bool r = true;
            readOperand(reg, v, t, r);
            if (is_first) {
                rs.src1_value = v; rs.src1_tag = t; rs.src1_ready = r;
            } else {
                rs.src2_value = v; rs.src2_tag = t; rs.src2_ready = r;
            }
        };

        switch (ins.op) {
            case OpCode::ADDI: case OpCode::SLTI: case OpCode::ANDI:
            case OpCode::ORI: case OpCode::XORI:
            ...
            case OpCode::LW:
                rs.is_load = true;
                fill_reg_src(ins.src1, true); // base
                rs.src2_ready = true;
                rs.src2_value = 0;
                break;
            case OpCode::SW:
                rs.is_store = true;
                fill_reg_src(ins.src1, true);  // base
                fill_reg_src(ins.src2, false); // store data
                break;
            default:
                fill_reg_src(ins.src1, true);
        ...
        if (unit_t == UnitType::LOADSTORE) lsq->enqueue(rs);
        else getUnit(unit_t)->enqueue(rs);

        if (writesRegister(ins.op) && ins.dest != 0) RAT[ins.dest] = tag;
        fetched.valid = false;
    }
    ...
    bool hasInFlightWork() const {
        if (fetched.valid) return true;
        if (rob_count > 0) return true;
        for (const auto& u : units) {
            if (!u.rs.empty() || !u.pipeline.empty()) return true;
        }
        if (!lsq->q.empty() || !lsq->inflight.empty()) return true;
        return false;
    }
    ...
    bool step() {
        ...
        if (no_more_program && !hasInFlightWork()) return false;
